...
struct Amp : Module {
    ...
    void process(const ProcessArgs &args) override {
        outputs[OUT_CH_ONE_OUTPUT].setVoltage(params[IN_CH_ONE_INPUT].getValue());
        outputs[OUT_CH_TWO_OUTPUT].setVoltage(params[IN_CH_TWO_INPUT].getValue() * 2);
        outputs[OUT_CH_THR_OUTPUT].setVoltage(params[IN_CH_THR_INPUT].getValue() * 10);
        outputs[OUT_CH_FOU_OUTPUT].setVoltage(params[IN_CH_FOU_INPUT].getValue() * 12);
    }
};
